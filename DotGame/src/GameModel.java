...
public class GameModel implements Cloneable, Serializable {
   ...
   /**
    ...
    public Object clone()
    {
        GameModel newModel = new GameModel(sizeOfGame);
        newModel.model = new int[sizeOfGame][sizeOfGame];
        for (int i = 0; i < sizeOfGame; i++)
        {
            for (int j = 0; j < sizeOfGame; j++)
            {
                newModel.model[i][j] = model[i][j];
            }
        }
        newModel.currentDot = (Point) currentDot.clone();
        newModel.numberOfSteps = numberOfSteps;
        return newModel;
    }
